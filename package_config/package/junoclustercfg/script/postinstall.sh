...
$INSTALL -m 755 -o $CURRENT_USER -g $group $prefix/$name/config-$name.toml $prefix/$name/config.toml
...
for FILE in start.sh stop.sh shutdown.sh \
    log.sh logstate.sh config.toml \
...
### run clustermgr tool, it will throw "[ERROR] store command is not allowed to update existing 
### cluster" which is fine. That means clustercfg has been loaded into etcd server already
${prefix}/${name}/clustermgr -config ${prefix}/${name}/config.toml -cmd store -type cluster_info
