...
import com.paypal.juno.util.SendBatch;
import java.util.*;
...
//	private final String INSTANCE_GEO_PP_US = "PP_US";
	 ...
	 */
	private static final Logger LOGGER = LoggerFactory.getLogger(JunoAsyncClientImpl.class);
	
	JunoAsyncClientImpl(JunoClientConfigHolder config,SSLContext ctx,boolean isAsync){
		this(config,ctx);
		this.isAsync = isAsync;
	}
	
	protected JunoAsyncClientImpl(JunoClientConfigHolder config,SSLContext ctx){
	...
	public Single<JunoResponse> create(byte[] key, byte[] value) throws JunoException {
		try{
			...
			return processSingle(req,OperationType.Create);
...
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
	...
	public Single<JunoResponse> update(byte[] key, byte[] value) throws JunoException {
		try{
			...
			return processSingle(req,OperationType.Update);
		}catch(JunoException je){
			throw je;
//		}catch (IllegalArgumentException iae) {
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
...
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
	...
	public Single<JunoResponse> set(byte[] key, byte[] value) throws JunoException {
		try{
			...
			return processSingle(req,OperationType.Set);
		}catch(JunoException je){
			throw je;
//		}catch (IllegalArgumentException iae) {
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
...
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
	...
	public Single<JunoResponse> delete(byte[] key) throws JunoException {
		try{
			...
			return processSingle(req,OperationType.Destroy);
		}catch(JunoException je){
			throw je;
//		}catch (IllegalArgumentException iae) {
//			throw new JunoException(OperationStatus.IllegalArgument.getErrorText(),iae);
	...
	public Single<JunoResponse> compareAndSet(RecordContext jcx, byte[] value, long timeToLiveSec) throws JunoException {
		try{
			...
			return processSingle(req,OperationType.CompareAndSet);
		}catch(JunoException je){
			throw je;
		}catch (IllegalArgumentException iae) {
	...
	@Override
	public Observable<JunoResponse> doBatch(final Iterable<JunoRequest> request) throws JunoException {
		SocketConfigHolder socConfig = new SocketConfigHolder(configHolder);
		...
		try{
			Observable<JunoResponse> resp = Observable.<JunoResponse>create(t-> {
					...
					try{
							...
							if(reqIdReqMsgMap.size() != 0){
								if(!operationRetry){
									...
									for(Map.Entry<UUID, JunoMessage> entry : reqIdReqMsgMap.entrySet()){
										...
										trans.put("req_attempt",String.valueOf(retryCounter));
										//trans.put("Operation",jMsg.getOpType().name());
						...
						JunoMetrics.recordOpTimer(JunoMetrics.JUNO_LATENCY_METRIC,
							"JUNO_BATCH", socConfig.getJunoPool(), JunoMetrics.ERROR,System.currentTimeMillis() - opStartTime);
						LOGGER.error("RootTxnStatus : {}", rootTrans);
						LOGGER.error(e.getMessage() +" ["+ this.configHolder.printProperties()+"]");
						t.onError(new JunoException(OperationStatus.InternalError.getErrorText(),e));
					}finally{
						opaqueResMap.remove(batchOpaque);
					}
	 ...
	 */
	private Single<JunoResponse> processSingle(final JunoRequest req,final OperationType opType){
		final Map<String,CharSequence> trans = new HashMap<String, CharSequence>();
		trans.put("isAsync", String.valueOf(isAsync));
		trans.put("USE_SSL", configHolder.getUseSSL()? JunoConstants.JUNO_SSL_CLIENT : JunoConstants.JUNO_CLIENT);
		...
		SocketConfigHolder socConfig = new SocketConfigHolder(configHolder);
		try{
			Single<JunoResponse> resp = Single.<JunoResponse>create(t-> {
				int opaque = 0;
					try{
