...
public class BatchUpdateTest{
	...
	public void testBatchUpdatePayloadLifetime() throws JunoException{
		...
		HashMap<String, Long> hmapVersion = new HashMap<String, Long>();
		...
		try {
			...
			for (JunoResponse mResponse: batchResp) {	
				...
				AssertJUnit.assertTrue(1 == mResponse.getVersion() || 2 == mResponse.getVersion());
				hmapVersion.put(mKey, mResponse.getVersion());
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			JunoRequest uitem = new JunoRequest(key[i], upayload[i], hmapVersion.get(new String(key[i])), ttl[i], JunoRequest.OperationType.Update);
			ulist.add(uitem);
		}
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			AssertJUnit.assertTrue(2 == junoResponse.getVersion() || 3 == junoResponse.getVersion() || 4 == junoResponse.getVersion());
	...
	public void testBatchUpdateEmptyKey() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			JunoRequest item = new JunoRequest(key[i], payload[i], (long)0, (long)0, JunoRequest.OperationType.Set);
                        JunoRequest updateItem = new JunoRequest(key[i], payload[i], (long)0, (long)0, JunoRequest.OperationType.Update);
			list.add(item);
                        updateList.add(updateItem);
		}
		try{
                        Iterable<JunoResponse> batchResp;
			LOGGER.debug("Update " + numKeys + " keys using batch Set first");
			if (syncFlag == 1) { 
			...
			LOGGER.info("Completed");
                ...
                try{    
                        ...
                        LOGGER.debug("Update " + numKeys + " keys using batch update");
                        if (syncFlag == 1) { 
                                updateResp=junoClient.doBatch(list).toBlocking().toIterable();
                        } else {
                                updateResp=BatchTestSubscriber.async_dobatch(asyncJunoClient, list);
                        }
		...
		try {
			...
			LOGGER.info("Completed");
                ...
                try {   
                        ...
                        if (syncFlag == 1) {
                                updateResp=junoClient.doBatch(updateList).toBlocking().toIterable();
                        } else {
                                updateResp=BatchTestSubscriber.async_dobatch(asyncJunoClient, updateList);
                        }
                        for (JunoResponse response: updateResp) {
                            if (response != null && response.getKey() != null && response.getKey() != key[9]) {
                                AssertJUnit.assertEquals (OperationStatus.Success, response.getStatus());
	...
	public void testBatchUpdateZeroPayload() throws JunoException{
		...
		LOGGER.debug("\n===Batch Update is sent ");
		if (syncFlag == 1) {				
			BlockingObservable<JunoResponse> resp = junoClient.doBatch(list1).toBlocking();
			resp.subscribe();
		} else {
			BatchTestSubscriber.async_dobatch(asyncJunoClient, list1);
		}
		
		List<JunoRequest> list2 = new ArrayList<>();
		for (int i = 0; i < numKeys; i ++) {		
			JunoRequest item2 = new JunoRequest(key[i], null, 0, 0, JunoRequest.OperationType.Get);
			list2.add(item2);
		}
		
		Iterable <JunoResponse> getBatchResponse;
		if (syncFlag == 1) {				
			getBatchResponse = junoClient.doBatch(list2).toBlocking().toIterable();
		} else {
			getBatchResponse = BatchTestSubscriber.async_dobatch(asyncJunoClient, list2);
		}
	...
	public void testBatchUpdateNullPayload() throws JunoException{
		...
		try {			
			if (syncFlag == 1) {
				BlockingObservable resp = junoClient.doBatch(list1).toBlocking();
				resp.subscribe();
			} else {
				BatchTestSubscriber.async_dobatch(asyncJunoClient, list1);
			}
	...
	public void testBatchUpdateMoreThan200KPayLoad() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			hmapTTL.put(new String(key[3]), (long)30);
			JunoRequest item1 = new JunoRequest(key[i], payload[i], (long)0, ttl[i], JunoRequest.OperationType.Update);
			list1.add(item1);
		}
	...
	public void testBatchUpdateMoreThan3Days() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			JunoRequest item1 = new JunoRequest(key[i], payload[i], (long)1, ttl[i], JunoRequest.OperationType.Update);
			list1.add(item1);
		}
		LOGGER.debug("\n===Batch Update is sent ");
		try {
	...
	public void testBatchUpdateNoLifetime() throws JunoException , InterruptedException {
		...
		Iterable<JunoResponse> getBatchResponse;
		if (syncFlag == 1) {
			getBatchResponse = junoClient.doBatch(glist).toBlocking().toIterable();
		} else {
			getBatchResponse = BatchTestSubscriber.async_dobatch(asyncJunoClient, glist);
		}
		int i = 0;
		for (JunoResponse response: getBatchResponse) {		
			AssertJUnit.assertEquals (OperationStatus.NoKey, response.getStatus());
	...
	public void testBatchUpdateVersion0() throws JunoException{
		...
		try {
			Iterable<JunoResponse> batchResp= null;
			if (syncFlag == 1) {
				batchResp = junoClient.doBatch(list2).toBlocking().toIterable();			
			} else {
				batchResp = BatchTestSubscriber.async_dobatch(asyncJunoClient, list2);
			}
			for (JunoResponse mResponse: batchResp) {
				AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
				String mKey = new String(mResponse.getKey());
	...
	public void testBatchUpdateInvalidVersion() throws JunoException{
		...
		try {
			Iterable<JunoResponse> batchResp = junoClient.doBatch(list2).toBlocking().toIterable();
			int i = 0; 
			for (JunoResponse mResponse: batchResp) {	
				String mkey = new String(mResponse.getKey());
		...
		try {
			Iterable<JunoResponse> batchResp = junoClient.doBatch(list3).toBlocking().toIterable();
			int i = 0; 
			for (JunoResponse mResponse: batchResp) {	
				String mkey = new String(mResponse.getKey());
