...
public class ReactorSetTest{
	...
	public void testsetWithKey() throws IOException{
		...
		JunoResponse mResponse = junoReactClient.set(key, data).block();
		...
		mResponse = junoReactClient.set(key, data1).block(); //ttl won't be updated
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		...
		JunoResponse junoResponse = junoReactClient.set(key,data2, lifetime).block();
		...
		junoResponse = junoClient.set(key, data3, (long)10);
		AssertJUnit.assertEquals(OperationStatus.Success, junoResponse.getStatus());
		AssertJUnit.assertTrue(4 == junoResponse.getVersion());
		AssertJUnit.assertTrue(lifetime == junoResponse.getTtl());
		
		junoResponse = junoReactClient.set(key, data4, (long)0).block();
		...
		JunoResponse junoResponse1 = junoReactClient1.set(key, data, (long)0).block(); //it gets default time from server side which is 3600
	...
	public void testsetWithSameKey() throws JunoException, IOException{
		...
		long lifetime = 100;
		JunoResponse junoResponse = junoReactClient.set(key, data).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		junoResponse = junoReactClient.get(key).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		AssertJUnit.assertEquals(key, junoResponse.key());
		...
		JunoResponse mResponse = junoReactClient1.set(key, data2, lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		AssertJUnit.assertTrue(1 == junoResponse.getVersion());
		AssertJUnit.assertTrue(key == junoResponse.getKey());	
		mResponse = junoReactClient1.get(key).block();
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
	...
	public void testsetWithoutTTL() throws JunoException, IOException {
		...
		junoResponse = junoReactClient.get(key).block(); // It will show that 10 sec has elapsed from original TTL
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		...
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		junoResponse = junoReactClient.get(key).block(); // It will show that 1800 sec as TTL.
		AssertJUnit.assertEquals(OperationStatus.Success, junoResponse.getStatus());
		AssertJUnit.assertEquals(key, junoResponse.key());
	...
	public void testSetWithEmptyKey() throws JunoException, IOException{
		...
		try{			
			mResponse = junoReactClient.set(key, data).block();
	...
	public void testsetWithNullKey() throws JunoException, IOException{
		...
		try{
			long lifetime = 100;
			junoReactClient.set(key, data, (long)lifetime).block();
	...
	public void testsetWith128BytesKey() throws JunoException, IOException{
		...
		byte[] key = new String(DataGenUtils.createKey(128)).getBytes();
		byte[] data = DataGenUtils.genBytes(10);
		long lifetime = 100;
		JunoResponse junoResponse = null;
		try {
			junoResponse = junoReactClient.set(key, data, lifetime).block();
	...
	public void testsetWith129BytesKey() throws JunoException, IOException{
		...
		byte[] data = DataGenUtils.genBytes(10);
		try{
			junoReactClient.set(key, data, (long)100).block();
	...
	public void testsetWithSpecialCharsKey() throws JunoException, IOException{
		...
		try {
			...
			AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
			junoResponse = junoReactClient.delete(key1).block();
			AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
			Thread.sleep(5000);
		} catch (Exception e) {
		...
		JunoResponse junoResponse1 = junoReactClient.set(key1, data1).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse1.getStatus());
		AssertJUnit.assertEquals(key1, junoResponse1.key());
		AssertJUnit.assertTrue(1 == junoResponse1.getVersion());
		junoResponse1 = junoReactClient.get(key1).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		...
		JunoResponse junoResponse2 = junoReactClient2.get(key).block();
	...
	public void testsetWith10SecLifetime() throws JunoException, IOException, InterruptedException {
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		Thread.sleep (2000);
		...
		Thread.sleep (4000);
		try {
			JunoResponse mResponse = junoReactClient2.get(key).block();
	...
	public void testsetWithZeroLifetime() throws JunoException, Exception{
		...
		JunoResponse mResponse = junoReactClient.set(key, data, (long)15).block();
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
		...
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
		AssertJUnit.assertTrue (mResponse.getTtl() <= prop.getDefaultLifetime() && mResponse.getTtl() > prop.getDefaultLifetime()-3);			
		mResponse = junoReactClient.set(key, data, (long)0).block();
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
		AssertJUnit.assertTrue (mResponse.getTtl() <= prop.getDefaultLifetime() && mResponse.getTtl() > prop.getDefaultLifetime()-3);
		AssertJUnit.assertEquals(3,mResponse.getVersion());
		mResponse = junoReactClient2.set(key, data2).block();
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
		...
		mResponse = junoReactClient1.set(key, data, (long)0).block();
		AssertJUnit.assertEquals (OperationStatus.Success, mResponse.getStatus());
	...
	public void testsetWith3DaysLifetime() throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, (long)lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());

		junoResponse = junoReactClient2.get(key, (long)10).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		AssertJUnit.assertEquals(key, junoResponse.key());
		AssertJUnit.assertTrue(1 == junoResponse.getVersion());
		AssertJUnit.assertEquals(data.length, junoResponse.getValue().length);
	...
	public void testsetWithNullPayload() throws JunoException, IOException{
		...
		byte[] key = DataGenUtils.genBytes(64);
		try{
			JunoResponse res = junoReactClient.set(key, null, (long)50).block();
	...
	public void testsetWithZeroPayload() throws JunoException, IOException{
		...
		byte[] key = DataGenUtils.genBytes(64);
		try{
			JunoResponse junoResponse = junoReactClient.set(key, data, (long)20).block();
			AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
			LOGGER.info("0");
		}catch(Exception mex){			
	...
	public void testsetWith200KBPayload() throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, (long)15).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
	...
	public void testsetWithMorethan200KBPayload() throws JunoException, IOException{
		...
		byte[] key = DataGenUtils.genBytes(64);
		try{
			junoReactClient.set(key, data, (long)10).block();
	...
	public void testsetWithSameKeyInDifferentNameSpaces() throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, (long)100).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		...
		junoResponse = junoReactClient1.set(key, data1).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
	...
	public void testsetWithSameKeysInSameNameSpace() throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, (long)10).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		...
		JunoResponse mResponse = junoReactClient2.set(key, data2, (long)100).block();
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		mResponse = junoReactClient.get(key).block();
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		AssertJUnit.assertTrue(2 == mResponse.getVersion());
	...
	public void testsetWithSameNameSpaceDiffAppname() throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.create(key, data, (long)100).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		...
		junoResponse = junoReactClient2.set(key, data2, (long)10).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		JunoResponse mResponse = junoReactClient2.get(key).block();
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		AssertJUnit.assertEquals(new String(data2), new String(mResponse.getValue()));
	...
	public void testSetWithOtherOps() throws JunoException, IOException{
		...
		JunoResponse mResponse = junoReactClient2.update(key, data2, (long)0).block();
		AssertJUnit.assertEquals (OperationStatus.Success,mResponse.getStatus());
		...
		long lifetime = 100L;
		JunoResponse junoResponse1 = junoReactClient.compareAndSet(mResponse.getRecordContext(), data3, lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse1.getStatus());
	...
	public void testCompareAndTestWithWrongVersion()throws JunoException, IOException{
		...
		JunoResponse junoResponse = junoReactClient.set(key, data1, (long)100).block();
		AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
		...
		try{
			 junoReactClient.compareAndSet(reqCtx, data2, (long)240).block();
	...
	public void testValidateByChangingMaxPayloadSize() throws JunoException, IOException{
		...
		JunoReactClient junoReactClient2 = null;
		try {
			junoReactClient2 = JunoClientFactory.newJunoReactClient(new JunoPropertiesProvider(pConfig), SSLUtil.getSSLContext());
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
		...
		try{
			junoReactClient2.set(key, data, (long)10).block();
	...
	public void testsetWithChineseKey() throws JunoException, Exception {		
		...
		JunoResponse junoResponse = junoReactClient.set(key, data, (long)lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		JunoResponse junoResponse1 = junoReactClient.set(key1, data1, (long)lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse1.getStatus());
		junoResponse =  junoReactClient.get(key).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		junoResponse1 =  junoReactClient.get(key1).block();
	...
	@Test
		...
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
		Thread.sleep (2000);
                junoResponse = junoReactClient.set(key, data, (long)lifetime).block();
		AssertJUnit.assertEquals (OperationStatus.Success, junoResponse.getStatus());
                junoResponse = junoReactClient.get(key).block();
                AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
                AssertJUnit.assertEquals(key, junoResponse.key());
		...
		try {
	                JunoResponse junoResponse1 = junoReactClient.update(key, updateD).block(); 
                 }catch(Exception mex){
                        LOGGER.debug(mex.getMessage());
                        AssertJUnit.assertTrue(mex.getCause().getMessage().contains("The Document Value must not be larger than 204800"));
                        LOGGER.info("Exception", mex.getMessage());
...
//				JunoResponse junoResponse = junoReactClient.set(key, data, lifetime);
//				AssertJUnit.assertEquals (OperationStatus.Success,junoResponse.getStatus());
//
//                                JunoResponse junoResponse1 = junoReactClient.get(key);
//                                AssertJUnit.assertEquals (OperationStatus.Success,junoResponse1.getStatus());
