...
public class BatchReactGetTest{
	...
	public void setup() throws  IOException, InterruptedException {
		...
		LOGGER = LoggerFactory.getLogger(BatchReactGetTest.class);

		URL url = BatchReactGetTest.class.getResource("/com/paypal/juno/Juno_batch.properties");
		...
		URL url1 = BatchReactGetTest.class.getResource("/com/paypal/juno/Juno_batch.properties");
		...
		URL url2 = BatchReactGetTest.class.getResource("/com/paypal/juno/Juno_batch.properties");
		...
		try{
			...
			junoReactClient2 = JunoClientFactory.newJunoReactClient(url);
	...
	public void testBatchGet() throws JunoException, InterruptedException{
		...
		try {
			...
			LOGGER.debug("reactASync is " + syncFlag.reactAsync.ordinal() + ", flag is " + flag);
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call, flag 1 is sync call, flag 0 is async call
				LOGGER.debug("enter into sync create call");
                                batchResp = junoReactClient.doBatch(list).toIterable();
                        } else {        //async react call
				LOGGER.debug("enter into async create call");
		...
		Iterable<JunoResponse> getBatchResp = null;
			...
			LOGGER.debug("enter into sync get call");
                        getBatchResp = junoReactClient.doBatch(list1).toIterable();
                } else {        //async react call
			LOGGER.debug("enter into async get call");
                        getBatchResp = BatchReactSubscriber.async_dobatch(junoReactClient, list1);
                }
	...
	public void testGetWithLifeTimeExtented() throws JunoException, InterruptedException {
		...
		Iterable<JunoResponse> getBatchResp1 = null;
                if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                        getBatchResp1 = junoReactClient.doBatch(list1).toIterable();
                } else {        //async react call
                        getBatchResp1 = BatchReactSubscriber.async_dobatch(junoReactClient, list1); 
                }
	...
	public void testBatchGetEmptyKey() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			JunoRequest item = new JunoRequest(key[i], payload[i], (long)0, ttl[i], JunoRequest.OperationType.Set);
                        JunoRequest getItem = new JunoRequest(key[i], payload[i], (long)0, (long)0, JunoRequest.OperationType.Get);
			list.add(item);
                        getList.add(getItem);
		}
		try{
                        Iterable<JunoResponse> batchResp;
			LOGGER.debug("Read " + numKeys + " keys using batch Set()");
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				batchResp=junoReactClient.doBatch(list).toIterable();
			} else {        //async react call
				batchResp=BatchReactSubscriber.async_dobatch(junoReactClient, list); 
			...
			LOGGER.info("Completed");
                ...
                try{    
                        ...
                        LOGGER.debug("Read " + numKeys + " keys using batch Get()");
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                getResp=junoReactClient.doBatch(list).toIterable();
                        } else {        //async react call
                                getResp=BatchReactSubscriber.async_dobatch(junoReactClient, list);
                        }
	...
	public void testBatchGetNotExist() throws JunoException{
		...
		Iterable<JunoResponse> getBatchResp1 = null;
                if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                        getBatchResp1 = junoReactClient.doBatch(list).toIterable();
                } else {        //async react call
                        getBatchResp1 = BatchReactSubscriber.async_dobatch(junoReactClient, list); 
                }
		for (JunoResponse response: getBatchResp1) {		
			String mkey = new String(response.getKey());
			LOGGER.debug("Key is??: " + mkey);
		...
		try{
			...
			LOGGER.info("Completed");
                ...
                try{    
                        ...
                        Iterable<JunoResponse> getResp;       
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                getResp=junoReactClient.doBatch(getList).toIterable();
                        } else {        //async react call
                                getResp=BatchReactSubscriber.async_dobatch(junoReactClient, getList);
                        }
                        for (JunoResponse response: getResp) {
                            if (response != null && response.getKey() != null) {
                                AssertJUnit.assertEquals(OperationStatus.Success, response.getStatus());
				AssertJUnit.assertEquals(1, response.getVersion());
                            } else {
                                AssertJUnit.assertEquals(OperationStatus.IllegalArgument, response.getStatus());
                            }
                        }
	...
	public void testBatchGetLifetimeLongerThan3Days() throws JunoException{
		...
		try {
			LOGGER.debug("Read " + numKeys + " keys using batch Get()");
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				gResp = junoReactClient.doBatch(list).toIterable();
			} else {        //async react call
				gResp = BatchReactSubscriber.async_dobatch(junoReactClient, list); 
			}
		} catch (JunoException mex) {
	...
	public void testBatchGetZeroItem() throws JunoException{
		...
		try {
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                junoReactClient.doBatch(list).toIterable();
                        } else {        //async react call
                                BatchReactSubscriber.async_dobatch(junoReactClient, list);
                        }
	...
	public void testBatchSetTTLmorethan3days() throws JunoException{
		...
		try {	
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				resp = junoReactClient.doBatch(list).toIterable();
			} else {        //async react call
				resp = BatchReactSubscriber.async_dobatch(junoReactClient, list); 
			}
		} catch (JunoException mex) {
