...
public class BatchReactSetTest{
	private JunoReactClient junoRctClient;
	...
	public void setup() throws  IOException, InterruptedException {
		...
		try{
			junoRctClient = JunoClientFactory.newJunoReactClient(new JunoPropertiesProvider(pConfig), SSLUtil.getSSLContext());
	...
	public void testBatchCreateSet() throws JunoException{
		...
		try {
			Iterable<JunoResponse> batchResp = null;
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                batchResp = junoRctClient.doBatch(list).toIterable();
                        } else {        //async react call
                                batchResp = BatchReactSubscriber.async_dobatch(junoRctClient, list);
                        }
			int i = 1;
		...
		Iterable<JunoResponse> getBatchResp;
                if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                        getBatchResp = junoRctClient.doBatch(list2).toIterable();
                } else {        //async react call
                        getBatchResp = BatchReactSubscriber.async_dobatch(junoRctClient, list2);
                }
		int i = 0;
	...
	public void testBatchSetEmptyKeys() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			JunoRequest item = new JunoRequest(key[i], payload[i], (long)0, (long)0, System.currentTimeMillis(), JunoRequest.OperationType.Set);
			list.add(item);
		}
		try{
	...
	public void testBatchSet257BytesKeys() throws JunoException
	{
		...
		try {
			...
			Iterable<JunoResponse> resp;
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                resp = junoRctClient.doBatch(list).toIterable();
                        } else {        //async react call
                                resp = BatchReactSubscriber.async_dobatch(junoRctClient, list);
                        }
                        for (JunoResponse mResponse: resp) {
				String mkey = new String(mResponse.getKey());
	                        if ( mkey.equals(new String(key[9])) || mkey.equals(new String(key[1])) ) {
		...
		try {
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				getBatchResponse = junoRctClient.doBatch(getList).toIterable();
			} else {        //async react call
				getBatchResponse = BatchReactSubscriber.async_dobatch(junoRctClient, getList);
			}
			int i = 0;
	...
	public void testBatchSetZeroPayload() throws JunoException{
		...
		for (int i = 0; i < numKeys; i ++) {
			...
			ttl[i] = 20;	
			hmap.put(new String(key[i]), payload[i]);
			LOGGER.debug("i is " + i + ", key is " + (new String(key[i])) + ", payload is " + (new String(payload[i])));
		...
		Iterable <JunoResponse> resp;	
                ...
                AssertJUnit.assertEquals(k, numKeys);
		...
		Iterable <JunoResponse> getBatchResponse;
                if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                        getBatchResponse = junoRctClient.doBatch(list1).toIterable();
                } else {        //async react call
                        getBatchResponse = BatchReactSubscriber.async_dobatch(junoRctClient, list1);
                }
	...
	public void testBatchSetNullPayload() throws JunoException{
		...
		try {
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				setBatchResponse = junoRctClient.doBatch(list1).toIterable();
			} else {        //async react call
				setBatchResponse = BatchReactSubscriber.async_dobatch(junoRctClient, list1);
			}
			int i = 0;
	...
	public void testBatchSetTTLmorethan3days() throws JunoException{
			...
			Iterable<JunoResponse> resp = null;	
				...
				if ( mkey.equals(new String(key[1]))) {
					...
					k++;
                        ...
                        AssertJUnit.assertEquals(k,numKeys-1);
		} catch (JunoException mex) {
		...
		try {
			...
			if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
				getBatchResp = junoRctClient.doBatch(list1).toIterable();
			} else {        //async react call
				getBatchResp = BatchReactSubscriber.async_dobatch(junoRctClient, list1);
			}
			...
			for (JunoResponse response: getBatchResp) {				
				...
				if (mkey.equals(new String(key[1]))) {
					...
					AssertJUnit.assertTrue(1 == response.getVersion());					
					AssertJUnit.assertTrue(hmapTTL.get(mkey)-10 <= response.getTtl()  && response.getTtl() <= hmapTTL.get(mkey));
					i++;
				}
			}
			AssertJUnit.assertEquals(i, numKeys-1);
	...
	public void testBatchSetZeroItem() throws JunoException{
		...
		try {
                        if (syncFlag.reactSync.ordinal() == flag) {     //sync react call
                                junoRctClient.doBatch(list).toIterable();
                        } else {        //async react call
                                BatchReactSubscriber.async_dobatch(junoRctClient, list);
                        }
